...
st.markdown(
    ...
    /* Fix for Streamlit form button width */
...
# --- Email Setup ---
# WARNING: Hardcoding credentials is insecure. Use Streamlit Secrets for production.
...
try:
    # It's better to initialize yagmail only when needed, or handle errors gracefully.
    ...
    yag = None
    logger.error(f"Failed to initialize yagmail: {str(e)}")
    st.warning(f"Could not connect to email server. Email alerts will be disabled. Error: {str(e)}")
...
# Helper function for parse_query
def map_keyword(keyword):
    return map_keyword_to_column(keyword, df.columns.tolist() if not df.empty else [])
...
def parse_query(query, columns):
    ...
    for pat, default_col in list_patterns:
        ...
        if match:
            col_keyword_groups = match.groups()
            col_keyword = col_keyword_groups[1] if len(col_keyword_groups) > 1 and col_keyword_groups[1] else \
                          col_keyword_groups[2] if len(col_keyword_groups) > 2 and col_keyword_groups[2] else 'employee ids'
...
def process_query(df_in, query_info):
    if df_in.empty:
        return pd.DataFrame({"Message": ["No data loaded to process query."]})
        
    filtered_df = df_in.copy()
    for col, op, val in query_info['conditions']:
        if col not in filtered_df.columns:
            return pd.DataFrame({"Error": [f"Column '{col}' not found for filtering."]})
        try:
            if op == '==':
            ...
            return pd.DataFrame({"Error": [f"Error during filtering on '{col}': {str(e)}"]})
    ...
    if query_info['operation'] == 'aggregate':
        ...
        if agg_col and agg_func:
            ...
            if agg_col not in filtered_df.columns:
                return pd.DataFrame({"Error": [f"Column '{agg_col}' not found for aggregation."]})
    ...
    if query_info['operation'] == 'group_aggregate':
        ...
        if group_col and agg_func:
            if group_col not in filtered_df.columns:
                return pd.DataFrame({"Error": [f"Column '{group_col}' not found for grouping."]})
            if agg_func == 'count':
    ...
    if query_info['operation'] == 'list_unique':
        ...
        if list_col:
            if list_col not in filtered_df.columns:
                return pd.DataFrame({"Error": [f"Column '{list_col}' not found for listing."]})
    ...
    if query_info['operation'] == 'group_top':
        ...
        if group_col and sort_col:
            if group_col not in filtered_df.columns or sort_col not in filtered_df.columns:
                return pd.DataFrame({"Error": [f"Column '{group_col}' or '{sort_col}' not found."]})
    ...
    if query_info['operation'] == 'sort':
        ...
        if sort_col:
            if sort_col not in filtered_df.columns:
                return pd.DataFrame({"Error": [f"Column '{sort_col}' not found for sorting."]})
            filtered_df[sort_col] = pd.to_numeric(filtered_df[sort_col], errors='coerce')
    ...
    if query_info['operation'] == 'filter':
        valid_columns = [col for col in query_info['columns'] if col in filtered_df.columns]
        if not valid_columns:
             return pd.DataFrame({"Message": ["No valid columns selected or no results found."]})
...
def get_chatbot_response(user_input, df, columns):
    ...
    try:
        ...
        if result_df.empty or 'Error' in result_df.columns or 'Message' in result_df.columns:
            ...
            response = f"Sorry, I cannot understand or answer this question. {suggestion} (Error: {error_msg})"
            st.session_state.chat_history.append({"role": "bot", "message": response})
            return response, None, None, None
        ...
        if query_info['operation'] == 'list_unique':
            ...
            group_col = query_info.get('group_by')
            sort_col = query_info.get('sort_column')
            response = f"Top employees by {sort_col} per {group_col} displayed in table."
        else:
...
if not df.empty:
    ...
    departments, job_titles, employee_ids = [], [], ['All']
    remote_options = ['All']
...
date_range = st.sidebar.date_input("Filter by Hire Date Range", [min_date, max_date])
...
with email_col1:
    if st.button("Email Low Performance Alert"):
        if yag:
            ...
            if low_perf_content != "None":
                try:
                    yag.send(to=receiver_admin_email, subject="🚨 Employee Alerts: Low Performance", contents=email_content)
                    st.success("✅ Low Performance alert email sent.")
                    logger.info("Low Performance email alert sent successfully.")
                except Exception as e:
                    st.error(f"Failed to send email: {str(e)}")
                    logger.error(f"Email sending error: {str(e)}")
            else:
                st.info("ℹ️ No low performance alerts to send.")
        else:
            st.error("Email client not initialized. Cannot send alert.")

with email_col2:
    if st.button("Email Low Satisfaction Alert"):
        if yag:
            ...
            if low_sat_content != "None":
                try:
                    yag.send(to=receiver_admin_email, subject="🚨 Employee Alerts: Low Satisfaction", contents=email_content)
                    st.success("✅ Low Satisfaction alert email sent.")
                    logger.info("Low Satisfaction email alert sent successfully.")
                except Exception as e:
                    st.error(f"Failed to send email: {str(e)}")
                    logger.error(f"Email sending error: {str(e)}")
            else:
                st.info("ℹ️ No low satisfaction alerts to send.")
        else:
            st.error("Email client not initialized. Cannot send alert.")

with email_col3:
    if st.button("Email High Retention Risk Alert"):
        if yag:
            ...
            if high_ret_content != "None":
                try:
                    yag.send(to=receiver_admin_email, subject="🚨 Employee Alerts: High Retention Risk", contents=email_content)
                    st.success("✅ High Retention Risk alert email sent.")
                    logger.info("High Retention Risk email alert sent successfully.")
                except Exception as e:
                    st.error(f"Failed to send email: {str(e)}")
                    logger.error(f"Email sending error: {str(e)}")
            else:
                st.info("ℹ️ No high retention risk alerts to send.")
        else:
            st.error("Email client not initialized. Cannot send alert.")
...
# Row 1: Remote Work Efficiency
row1_col1 = st.columns(1)
with row1_col1[0]:
...
# Row 2: Performance Treemap and Retention Risk
row2_col1, row2_col2 = st.columns(2)
with row2_col1:
    # st.markdown("**Performance Level Distribution by Job Title**") # Removed this line
    ...
    # --- START: Fix for NaN values in Treemap ---
    tree_data_copy = tree_data.copy()
    if not tree_data_copy.empty:
        ...
        tree_data_copy['Job_Title'] = tree_data_copy['Job_Title'].replace('nan', 'Unknown Job Title')
        tree_data_copy['Performance_Level'] = tree_data_copy['Performance_Level'].replace('nan', 'Unknown')
    color_map = {
        'Low': '#FF4040', 
        'Medium': '#FFA500', 
        'High': '#228B22',
    ...
    # --- END: Fix for NaN values in Treemap ---
    ...
    fig_tree = px.treemap(
        ...
        title="Performance Level Distribution by Job Title" # Added title here
    ) if not tree_data_copy.empty else px.treemap()
...
# Row 3: Pie, Avg Satisfaction, Trend
...
st.markdown("Ask about employee details (e.g., 'Employee 123', 'Average salary for analyst'), greetings, or general questions.")
...
with st.form(key="chat_form"):
    ...
    with col1:
        user_input = st.text_input("Type your question:", key="chat_input", label_visibility="collapsed", placeholder="Type your question...")
    with col2:
...
if submit_button and user_input:
    ...
    # --- VISUALIZATION BLOCK REMOVED ---
    # The code that previously asked "Would you like a visualization?"
    # and displayed st.radio() has been removed as requested.
